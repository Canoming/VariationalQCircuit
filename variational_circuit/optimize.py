...
def  vcirc_test(x,statein,vcirc,test_func=sep_purity,ansatz_li=None,*args,**kwargs):
...
def  __vcirc_test_neg(x,statein,vcirc,test_func=sep_purity,ansatz_li=None,*args,**kwargs):
    vcirc.update_ansatzes(x,ansatz_li)
    N = vcirc.N
    if (statein.dims[0] != [2]*N):
        raise ValueError("Invalid input state, must be state on %s qubits system." % N)
...
def circ_minimize(x0,statein,vcirc,test_func=sep_purity,*args,ansatz_li=None,opt_method="BFGS",
...
def circ_maximize(x0,statein,vcirc,test_func=sep_purity,*args,ansatz_li=None,opt_method="BFGS",
    jac=None, hess=None, hessp=None, bounds=None,
    constraints=(), tol=None, callback=None, options=None):
